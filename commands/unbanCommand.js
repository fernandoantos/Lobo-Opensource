...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission('BAN_MEMBERS')) return message.channel.send('Está faltando a permissão ** BAN_MEMBERS **!').then(m => m.delete({ timeout: 5000 }));
        ...
        message.guild.fetchBans().then( bans => {
            ...
            if (user) {
                embed.setTitle(`Desbanido com sucesso ${user.user.tag}`)
                    ...
                    .addField('Tag do usuário', user.user.tag, true)
                message.guild.members.unban(user.user.id, reason).then(() => message.channel.send(embed))
            } else {
                embed.setTitle(`usuario ${member.tag} não está banido!`)
                    .setColor('#ff0000')
