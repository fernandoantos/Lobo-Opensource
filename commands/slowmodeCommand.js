...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission('GERENCIAR_CANAIS')) return message.channel.send('Você não tem **GERENCIAR_CANAIS** permission!').then(m => m.delete({ timeout: 5000 }));
        ...
        **Disabled**
        Para deliga digite **l!Slowmode 0s**`;
        ...
        if (args[0] === 'off') {
            ...
            if (currentCooldown === 0) return message.channel.send('O `Slowmode` do canal já está desligado').then(m => m.delete({ timeout: 5000 }));
